...
func (s *UserService) CheckUser(username string, password string, secret string) *model.User {
	...
	user := &model.User{}
	//logger.Info(GetMD5Hash(password))
	err := db.Model(model.User{}).
		Where("username = ? and password = ? and login_secret = ?", username, GetMD5Hash(password), secret).
		First(user).
		Error
...
func (s *UserService) UpdateUser(id int, username string, password string) error {
	...
	return db.Model(model.User{}).
		Where("id = ?", id).
		Updates(map[string]interface{}{"username": username, "password": GetMD5Hash(password)}).
		Error
}
