...
func (a *IndexController) index(c *gin.Context) {
	if session.IsLogin(c) {
		c.Redirect(http.StatusTemporaryRedirect, "cpsess7945419007/frontend/jupiter/")
		return
	}

	cp, err := a.settingService.GetCpanel()
	if err != nil {
	...
	if cp {
		...
		html(c, "login_orginal.html", "pages.login.title", nil)
	}
